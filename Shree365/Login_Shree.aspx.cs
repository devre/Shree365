...
namespace Shree365
{
    public partial class Login_Shree : System.Web.UI.Page
    {
        ...
        //    sqlConn.ConnectionString = System.Configuration.ConfigurationManager.AppSettings["ConnectionString"].ToString();        
            string sSQL = "SELECT  EmailAddress, LastName, RoleId, (select RoleName from UserRole_TB AS Roles where Roles.RoleId = Users.RoleId) as rolename FROM Users_TB Users WHERE Status='1' AND EmailAddress = '" + txtLogin.Text.ToLower().Trim() + "' AND UserPassword='" + txtPassword.Text + "'";
            //SELECT  EmailAddress, RoleId, (select RoleName from UserRole_TB AS Roles where Roles.RoleId = Users. RoleId) as rolename FROM Users_TB Users WHERE EmailAddress = 'guest@guest.com' AND UserPassword='guest'
          ...
          //  objdata.Users_TBs.Select(x => x.UserName == txtLogin.Text.ToLower().Trim()).FirstOrDefault();
    ...
    //        if (ds.Tables.Count!=0 && ds.Tables[0].Rows.Count!=0)
            if (ds.Tables.Count != 0)
            {
               ...
               // divErrorMsg.InnerText = "Good Loged In -  for user" + txtLogin.Text;
               //string sSQL = "SELECT  UserName, EmailAddress, RoleId, (select RoleName from UserRole_TB AS Roles where Roles.RoleId = Users.RoleId) as rolename FROM Users_TB Users WHERE EmailAddress = '" + txtLogin.Text.ToLower().Trim() + "' AND UserPassword='" + txtPassword.Text + "'";
                //SqlCommand sqlComm = new SqlCommand(sSQL, sqlConn);
